...
InputGLFW* InputGLFW::last_instance = nullptr;
...
void InputGLFW::key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    Controls::get()->setKeyState(last_instance->keymap[key], keystate);
}
...
void InputGLFW::mouse_button_callback(GLFWwindow* window, int button, int action, int mods) {
    ...
    Controls::get()->setMouseState(last_instance->mousebtnmap[button], mousebtnstate);
}
